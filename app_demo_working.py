...
def task_page():
    ...
    with st.form("login_form"):
        ...
        submit_button = st.form_submit_button(label=":material/check: Submit")
...
def content_screen():
    ...
    if st.session_state['task_selected_state'] == "Compare changes between data sources":
        ...
        with st.expander("Click to view the data", expanded=True):
            ...
            # try:
            #     with col1:
            #         demand_type_selected = st.multiselect("Select the Demand Type:", dfs['Demand Type'].unique(), default=['BOOKED', 'SHIPPED'])
            #         dfs_demand_type_selected = dfs[dfs['Demand Type'].isin(demand_type_selected)]
            #         dfs2_demand_type_selected = dfs2[dfs2['Demand Type'].isin(demand_type_selected)]
            #         empty_columns = dfs_demand_type_selected.columns[dfs_demand_type_selected.isna().all()]
            #         empty_columns2 = dfs2_demand_type_selected.columns[dfs2_demand_type_selected.isna().all()]
            #         dfs_demand_type_selected.drop(columns=empty_columns, inplace=True)
            #         dfs_demand_type_selected.reset_index(drop=True, inplace=True)
            #         dfs2_demand_type_selected.drop(columns=empty_columns2, inplace=True)
            #         dfs2_demand_type_selected.reset_index(drop=True, inplace=True)
            #     with col2:
            #         business_unit_selected = st.multiselect("Select the Business Unit:", dfs['Business Unit'].unique(), default=['WS'])
            #         dfs = dfs_demand_type_selected[dfs_demand_type_selected['Business Unit'].isin(business_unit_selected)]
            #         dfs2 = dfs2_demand_type_selected[dfs2_demand_type_selected['Business Unit'].isin(business_unit_selected)]
            ...
            #         dfs2.drop(columns=empty_columns, inplace=True)
            #         dfs2.reset_index(drop=True, inplace=True)
            # except:
            #     st.warning("Please select both the demand type and business unit to analyze the data")
        ...
        if flag == 1:    
            if prompt := st.chat_input("Write your lines here..."):
                ...
                # ALWAYS be sure to exclude BOARD end customer unless asked. Be sure to always use fiscal dates and show this in the title. 
        ...
        with st.expander("Click to view the data", expanded=True):
            ...
            # current_date = date.today()
            # current_date_pandas = pd.to_datetime(current_date)
            # st.write(df_time_profile)
            # st.write(current_date_pandas)
            # st.write(df_time_profile[df_time_profile['Day'] == current_date_pandas])
            # st.write(df_time_profile['Day'].iloc[0])
            st.sidebar.divider()
            if st.sidebar.button(":material/menu: Anaplan View"):
                df_time_profile = pd.read_csv(os.path.abspath("data/Time Profile for BOT.csv"))
                df_time_profile['Day'] = pd.to_datetime(df_time_profile['Day'])
        ...
        if prompt := st.chat_input("Write your lines here..."):
            ...
            # Be sure to always use fiscal dates and show this in the title. Exclude BOARD end customer by default.
...
if __name__ == "__main__":
    ...
    #         if st.sidebar.button("Logout", key="logout_button_latest"):
    #             # Clear session state and local storage on logout
    #             # local_storage.deleteAll()
    #             st.session_state.clear()
