...
memory_location_files = st.secrets['DATABASE']['BUCKET_FILES']
...
def aws_credentials_files():
    # AWS S3 Bucket details
    bucket_name = memory_location_files
...
def get_aws_files():
    ...
    result = s3.list_objects_v2(Bucket=memory_location_files, Prefix='allocation_files/')
...
bucket_name_files, s3_allocation_file_path = aws_credentials_files()
...
def content_screen():
    ...
    encryption_key = st.secrets['DATABASE']['ENCRYPTION_KEY']  # Or load from a secure file if necessary
    ...
    tab_selected = st.sidebar.tabs(["Chat History", "Feedback"])
    ...
    @st.cache_data
    def load_encrypted_csv_aws(file_name):
        response = s3.get_object(Bucket=st.secrets['DATABASE']['BUCKET_FILES'], Key=f'allocation_files/{file_name}')
        encrypted_content = response['Body'].read()
        decrypted_content = cipher_suite.decrypt(encrypted_content)
        csv_content = decrypted_content.decode('ISO-8859-1')
        output = pd.read_csv(StringIO(csv_content), encoding='ISO-8859-1')
    ...
    with st.expander("Click to view the data", expanded=True):
        ...
        dfs = load_encrypted_csv_aws(file_name=first_file)
    ...
    with tab_selected[0]:
        ...
        st.write(f"**Chat:** *{st.session_state.conversation_selected}*")
